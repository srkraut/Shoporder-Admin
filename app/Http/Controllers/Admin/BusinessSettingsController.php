...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        elseif($name=='qr_payment') {
            $payment = BusinessSetting::where('key', 'qr_payment')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'key'        => 'qr_payment',
                    'value'      => json_encode([
                ...
                DB::table('business_settings')->where(['key' => 'qr_payment'])->update([
                    'key'        => 'qr_payment',
                    'value'      => json_encode([
                        'status' => $request['status'],
                    ]),
         ...
         elseif ($name == 'digital_payment') {
            ...
            if (isset($payment) == false) {
                ...
                DB::table('business_settings')->where(['key' => 'esewa'])->update([
                    ...
                    'value'      => json_encode([
                        'status'        => $request['status'],
                        'esewa_merchant_code'       => $request['esewa_merchant_code'],
                        'is_sandbox' => $request['is_sandbox'],
                    ]),
                    'updated_at' => now(),
                ]);
    ...
    public function viewSocialLogin()
    {
        ...
        if(!$data) {
            Helpers::insert_business_settings_key('social_login','[{"login_medium":"google","client_id":"","client_secret":"","status":"0"},{"login_medium":"facebook","client_id":"","client_secret":"","status":"0"}]');
            $data = BusinessSetting::where('key', 'social_login')->first(); 
        }
